...
class SearchResult extends Component {
	...
	linkTo(item){
		const link = `/admin/p/manageMembers/details/${item.attributes.id}`
		return (
			<React.Fragment>
				<Row>
					<Col>
	...
	removeMembership(accountId){
		let currenContainerId = commons.getWorkingContainerId()
		this.setState({
			working: false
		})
		
		accountService
		.removeContainerMembership(accountId, currenContainerId)
		.then(json => {
			this.setState({
				working: true
			})
